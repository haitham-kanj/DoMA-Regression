...
dset = 3;       % choice of data set --> see bellow
...
for run = 1:n_run
    ...
    toc
    %% PL D.C
    %f_hat = auto_tune_dmax_fit(y_train,X_train);
    %y_hat_test = f_hat(X_test);
    %MSE_Dmax(run) = mean((y_hat_test-y_test).^2)/var(y);
...
fprintf("\n\n %s = %.1f  -/+  %.1f \n","D-Max Affine ",...
    100*round(mean(MSE_Dmax),3),100*round(zn*std(MSE_Dmax)/sqrt(n_run),3));
...
%% Save
save("ResultsDmax\DataIndexDC"+string(dset)+".mat",'n_run','MSE_Dmax','MSE_DC','MSE_MARS','MSE_MLP','MSE_KNN')
